...
class T0002_rackhd_OSInstallUbuntu(CBaseCase):
    '''
            ...
            for node_id, node in node_os_install_dic.iteritems():
                ...
                if rsp.status == "running":
                    ...
                    if (elapse_run_time < 1215 and elapse_run_time > 1185) \
                        ...
                        ssh = self.monorail.obj_ssh_agent
                        ssh.remote_shell("ipmitool -I lanplus -U admin -P admin -H {} chassis power cycle".
                                         format(node_bmc_ip[node.id]))
    ...
    def deconfig(self):
        ...
        #ssh = self.monorail.obj_ssh_agent                                                      
        #rsp = ssh.remote_shell("bash +x rm_disk00.sh ip.txt")
